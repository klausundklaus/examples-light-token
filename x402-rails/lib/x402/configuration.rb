...
module X402
  class Configuration
    attr_accessor :wallet_address, :facilitator, :chain, :currency, :optimistic,
                  ...
                  :accepted_payments, :use_light_token, :token_program, :light_token_rpc_url
    ...
    def initialize
      ...
      @custom_tokens = {}
      @accepted_payments = []
      @use_light_token = ENV.fetch("X402_USE_LIGHT_TOKEN", "false") == "true"
      @token_program = ENV.fetch("X402_TOKEN_PROGRAM", nil)
      @light_token_rpc_url = ENV.fetch("X402_LIGHT_TOKEN_RPC_URL", nil)
    end
