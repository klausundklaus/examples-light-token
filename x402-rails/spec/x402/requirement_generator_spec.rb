...
RSpec.describe X402::RequirementGenerator do
  ...
  describe ".generate" do
    ...
    context "with Light Token mode enabled" do
      ...
      it "includes feePayer and tokenProgram in extra field for solana-devnet" do
        result = described_class.generate(
          amount: amount,
          resource: resource,
          chain: "solana-devnet"
      ...
      it "includes feePayer and tokenProgram in extra field for solana mainnet" do
        result = described_class.generate(
          amount: amount,
          resource: resource,
          chain: "solana"
      ...
      it "uses custom token_program when configured" do
        X402.configuration.token_program = "CustomTokenProgram123"
        ...
        expect(requirement[:extra][:tokenProgram]).to eq("CustomTokenProgram123")
      end

      it "allows fee_payer override for Light Token" do
        result = described_class.generate(
          amount: amount,
          resource: resource,
          chain: "solana-devnet",
          fee_payer: "CustomLightTokenFeePayer"
        )
        requirement = result[:accepts].first

        expect(requirement[:extra][:feePayer]).to eq("CustomLightTokenFeePayer")
        expect(requirement[:extra][:tokenProgram]).to eq("cTokenmWW8bLPjZEBAUgYy3zKxQZW6VKi7bqNFEVv3m")
      end
    end
