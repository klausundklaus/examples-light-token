...
RSpec.describe X402::PaymentValidator do
  ...
  describe "#validate" do
    ...
    context "with Light Token mode enabled" do
      before do
      ...
      let(:light_token_requirement) do
        X402::PaymentRequirement.new(
          scheme: "exact",
          network: "solana-devnet",
          max_amount_required: 1000,
      ...
      let(:light_token_payment_payload) do
        X402::PaymentPayload.new(
          scheme: "exact",
          network: "solana-devnet",
          payload: {
            transaction: "base64_encoded_light_token_transaction_here"
          }
        )
      end

      it "identifies Light Token chain correctly when use_light_token is true" do
        expect(light_token_payment_payload.light_token_chain?).to be true
        expect(light_token_payment_payload.solana_chain?).to be true
        expect(light_token_payment_payload.evm_chain?).to be false
      end

      it "returns transaction for Light Token payloads" do
        expect(light_token_payment_payload.transaction).to eq("base64_encoded_light_token_transaction_here")
      end

      it "skips recipient validation for Light Token (handled by facilitator)" do
        allow(facilitator_client).to receive(:verify).and_return({
          "isValid" => true,
          "payer" => "LightTokenPayerAddress"
        })

        result = validator.validate(light_token_payment_payload, light_token_requirement)
        expect(result[:valid]).to be true
        expect(result[:payer]).to eq("LightTokenPayerAddress")
      end

      context "when facilitator validates Light Token transaction" do
        before do
          allow(facilitator_client).to receive(:verify).and_return({
            "isValid" => true,
            "payer" => "LightTokenPayerPubkey"
          })
        end

        it "forwards payload to facilitator for verification" do
          expect(facilitator_client).to receive(:verify).with(
            light_token_payment_payload,
            light_token_requirement.to_h
          )
          validator.validate(light_token_payment_payload, light_token_requirement)
        end
      end
    end
  end
