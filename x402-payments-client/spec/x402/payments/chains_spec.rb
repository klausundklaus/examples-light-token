...
RSpec.describe X402::Payments do
  ...
  describe ".light_token_chain?" do
    ...
    it "returns false for Solana chains when use_light_token is false" do
      ...
      expect(X402::Payments.light_token_chain?("solana-devnet")).to be false
      expect(X402::Payments.light_token_chain?("solana")).to be false
    end

    it "returns true for Solana chains when use_light_token is true" do
      X402::Payments.configuration.use_light_token = true
      expect(X402::Payments.light_token_chain?("solana-devnet")).to be true
      expect(X402::Payments.light_token_chain?("solana")).to be true
    end

    it "returns false for EVM chains even when use_light_token is true" do
      X402::Payments.configuration.use_light_token = true
      expect(X402::Payments.light_token_chain?("base-sepolia")).to be false
      expect(X402::Payments.light_token_chain?("base")).to be false
    end
  end
  ...
  describe ".token_program_for" do
    ...
    it "returns nil when use_light_token is false" do
      X402::Payments.configuration.use_light_token = false
      expect(X402::Payments.token_program_for("solana-devnet")).to be_nil
    end

    it "returns default cToken program ID when use_light_token is true" do
      X402::Payments.configuration.use_light_token = true
      expect(X402::Payments.token_program_for("solana-devnet")).to eq("cTokenmWW8bLPjZEBAUgYy3zKxQZW6VKi7bqNFEVv3m")
    end
    ...
    it "returns custom token_program when configured" do
      ...
      expect(X402::Payments.token_program_for("solana-devnet")).to eq("CustomTokenProgram123")
    end
  end
