  ...
  module Payments
    ...
    class << self
      ...
      def rpc_url_for(chain_name)
        ...
        # For Light Token mode, prefer light_token_rpc_url
        if light_token_chain?(chain_name) && config.light_token_rpc_url
          return config.light_token_rpc_url
        end
      ...
      def light_token_chain?(chain_name)
        solana_chain?(chain_name) && X402::Payments.configuration.use_light_token
      end
      ...
      def fee_payer_for(chain_name)
        ...
        builtin = CHAINS[chain_name]
        return builtin[:fee_payer] if builtin

        chain_config(chain_name)[:fee_payer]
      end

      def token_program_for(chain_name)
        return nil unless light_token_chain?(chain_name)
        X402::Payments.configuration.token_program || "cTokenmWW8bLPjZEBAUgYy3zKxQZW6VKi7bqNFEVv3m"
      end
    end
  end
end
