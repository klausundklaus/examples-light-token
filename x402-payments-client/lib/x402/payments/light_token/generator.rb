...
module X402
  module Payments
    module LightToken
      ...
      CTOKEN_PROGRAM_ID = "cTokenmWW8bLPjZEBAUgYy3zKxQZW6VKi7bqNFEVv3m"
      ...
      class Generator
        ...
        def generate_header(amount:, resource:, description: nil, network: nil, private_key: nil, pay_to: nil, version: nil, fee_payer: nil)
          ...
          raise ConfigurationError, "Light Token requires a Solana chain with use_light_token=true" unless X402::Payments.light_token_chain?(chain_name)
          ...
          raise ConfigurationError, "Facilitator fee payer is required for Light Token" if facilitator_fee_payer.nil? || facilitator_fee_payer.empty?
          ...
          # Derive CToken ATAs using cToken program ID
          source_ata = derive_ctoken_associated_token_address(mint_address, sender_pubkey)
          destination_ata = derive_ctoken_associated_token_address(mint_address, recipient)

          serialized_tx = build_ctoken_transfer_transaction(
        ...
        # Derive CToken Associated Token Address
        # Uses cToken program ID for both token program and associated token program
        # Seeds: [owner, CTOKEN_PROGRAM_ID, mint] + CTOKEN_PROGRAM_ID + "ProgramDerivedAddress"
        def derive_ctoken_associated_token_address(mint, owner)
          mint_bytes = SolanaRuby::Utils.base58_to_bytes(mint)
          owner_bytes = SolanaRuby::Utils.base58_to_bytes(owner)
          ctoken_program_bytes = SolanaRuby::Utils.base58_to_bytes(CTOKEN_PROGRAM_ID)

          nonce = 255
          loop do
            buffer = owner_bytes.pack("C*") +
                     ctoken_program_bytes.pack("C*") +
                     mint_bytes.pack("C*") +
                     [nonce].pack("C") +
                     ctoken_program_bytes.pack("C*") +
        ...
        def build_ctoken_transfer_transaction(source_ata:, destination_ata:, owner:, amount:, recent_blockhash:, keypair:, fee_payer:)
          ...
          transaction.add_instruction(build_ctoken_transfer_instruction(
            source_ata: source_ata,
            destination_ata: destination_ata,
            owner: owner,
            amount: amount
        ...
        # Build CToken transfer instruction
        # Format: discriminator (1 byte) = 3 + amount (u64, 8 bytes)
        # Accounts: source (writable), destination (writable), authority (signer)
        def build_ctoken_transfer_instruction(source_ata:, destination_ata:, owner:, amount:)
          # CToken transfer discriminator is 3, followed by amount as u64
          data = ([3].pack("C") + [amount].pack("Q<")).bytes
