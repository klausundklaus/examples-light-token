...
pub use light_token::constants::LIGHT_TOKEN_CPI_AUTHORITY as CPI_AUTHORITY_PDA;
pub use light_token::instruction::{LIGHT_TOKEN_CONFIG as COMPRESSIBLE_CONFIG_V1, LIGHT_TOKEN_RENT_SPONSOR as RENT_SPONSOR};
...
pub mod setup {
    ...
    ///
    /// Derives a per-program rent sponsor PDA from `[RENT_SPONSOR_SEED]` + `program_id`.
    /// Returns `(config_pda, rent_sponsor_pda)`.
    pub async fn initialize_rent_free_config<R: Rpc + TestRpc>(
        ...
        // Derive per-program rent sponsor PDA
        ...
        // Fund the per-program rent sponsor PDA so it can pay for PDA rent
        rpc.airdrop_lamports(&rent_sponsor, 100_000_000_000)
            .await
            .expect("Fund rent sponsor PDA should succeed");

        println!("Rent-free config initialized at: {:?}", config_pda);
        println!("  Rent sponsor PDA: {:?} (funded)", rent_sponsor);
        (config_pda, rent_sponsor)
    }
...
pub mod light_tokens {
    ...
    use light_token::instruction::{
        derive_token_ata, find_mint_address, CompressibleParams, CreateAssociatedTokenAccount,
        TokenDataVersion,
    };
    ...
    ///
    /// This creates a new light mint with metadata.
    ...
    /// * `compression_config` - The config PDA (from initialize_rent_free_config)
    ///
    /// # Returns
    ...
    pub async fn mint_light_tokens<R: Rpc + Indexer>(
        ...
        let destination_ata = derive_token_ata(destination_owner, mint);
        ...
        rpc.create_and_send_transaction(&[create_ata_ix], &payer.pubkey(), &[payer])
            .await
            .expect("Create destination ATA should succeed");
    ...
    ///
    /// This creates a light token account for the given owner.
    ...
    /// Derive the Light token ATA address for an owner and mint
    pub fn derive_light_token_ata(owner: &Pubkey, mint: &Pubkey) -> Pubkey {
...
pub mod spl_interface {
    ...
    /// Transfer tokens from an SPL/T22 token account to a Light token account
    ///
    /// This transfers SPL tokens from a standard SPL/T22 ATA to a light token account.
    ///
    /// # Arguments
    /// * `rpc` - RPC client
    /// * `payer` - Transaction fee payer
