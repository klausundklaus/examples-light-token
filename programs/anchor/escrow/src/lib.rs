...
pub mod escrow {
    ...
    pub fn take_offer(ctx: Context<TakeOffer>, params: TakeOfferParams) -> Result<()> {
        instructions::take_offer::send_wanted_tokens_to_maker(&ctx, &params)?;
        instructions::take_offer::withdraw_from_vault(&ctx, &params)?;
