...
use token_transfer::{TransferParams, ID};
...
async fn test_transfer() {
    let config = ProgramTestConfig::new_v2(true, Some(vec![("token_transfer", ID)]))
    ...
    let transfer_accounts = token_transfer::accounts::Transfer {
        payer: payer.pubkey(),
    ...
    let transfer_ix = Instruction {
        ...
        .concat(),
        data: token_transfer::instruction::Transfer {
