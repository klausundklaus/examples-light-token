//! Example program demonstrating #[light_account(init, associated_token, ...)] macro usage.
//!
//! Creates a Light Protocol associated token account using the macro-based approach.
...
pub struct CreateAssociatedTokenAccountParams {
    ...
    /// Bump for the associated token account PDA
    pub associated_token_account_bump: u8,
}

/// Accounts struct for creating a Light Protocol associated token account.
...
pub mod light_token_macro_create_associated_token_account {
    ...
    /// Create a Light Protocol associated token account.
    /// The associated token account is created by the LightFinalize trait implementation
    ...
    pub fn create_associated_token_account<'info>(
        ...
        // Associated token account creation is handled by the macro-generated LightFinalize implementation.
        Ok(())
    }
}
