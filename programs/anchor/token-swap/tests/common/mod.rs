//! AMM test setup for 6 token standard combinations: SPL, Token 2022, Light Token.
//!
...
pub enum TokenConfig {
    /// SPL mint + SPL associated token accounts and Light Token vault. Transfers via `TransferInterfaceCpi`.
    Spl,
    /// Token 2022 mint + Token 2022 associated token accounts and Light Token vault. Transfers via `TransferInterfaceCpi`.
    Token2022,
...
pub async fn setup_amm_test<R: Rpc + TestRpc + Indexer>(
    ...
    let (depositor_ata_a, depositor_ata_b) = match config {
        ...
        TokenConfig::LightSpl => {
            // Tokens route through temp SPL associated token accounts because Light Token accounts
        ...
        TokenConfig::LightT22 => {
            // Tokens route through temp Token 2022 associated token accounts because Light Token accounts
...
/// - `Spl` / `Token2022`: create standard associated token accounts, mint directly
/// - `Light` / `LightToLight`: create associated Light Token accounts via `mint_light_tokens` (creates + mints in one call)
/// - `LightSpl` / `LightT22`: create temp SPL/Token 2022 associated token accounts, mint, then
