...
(async function () {
    ...
    // Create a mint first
    const { mint, transactionSignature: mintTx } = await createMintInterface(
        rpc,
        payer,
        payer,
        null,
        ...
        undefined,
        undefined,
        createTokenMetadata("ATATest", "ATAT", "https://example.com/metadata.json")
    );
    console.log("Mint:", mint.toBase58());
    console.log("Mint tx:", mintTx);

    // Create an ATA for the payer
    const ata = await createAtaInterface(rpc, payer, mint, payer.publicKey);
